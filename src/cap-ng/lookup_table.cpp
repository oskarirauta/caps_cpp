...
static int capng_lookup_name(const transtab *table, const char *tabstr, size_t length, const char *name) {
...
static const char *capng_lookup_number(const transtab *table, const char *tabstr, size_t length, int number) {
...
int capng_name_to_capability(const char *name) {
	...
	return capng_lookup_name(captab, captab_msgstr.str, CAP_NG_CAPABILITY_NAMES, name);
}

static char *ptr2 = nullptr;
const char *capng_capability_to_name(unsigned int capability) {
	...
	ptr = capng_lookup_number(captab, captab_msgstr.str, CAP_NG_CAPABILITY_NAMES, capability);
	if ( ptr == nullptr ) { // This leaks memory, but should almost never be used
		free(ptr2);
