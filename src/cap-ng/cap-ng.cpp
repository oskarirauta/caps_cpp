...
#include "cap-ng/cap-ng-config.hpp"
...
enum capng_states_t { CAPNG_NEW, CAPNG_ERROR, CAPNG_ALLOCATED, CAPNG_INIT, CAPNG_UPDATED, CAPNG_APPLIED };

// Create an easy data struct out of the kernel definitions
typedef union {
	__user_cap_data_struct v1;
	__user_cap_data_struct v3[VFS_CAP_U32];
} cap_data_t;
...
static void init_lib(void) {
...
#ifdef HAVE_PTHREAD_H
	pthread_atfork(nullptr, nullptr, deinit);
#endif
	...
	if ( last_cap == 0 ) {
		...
		if ( fd = open("/proc/sys/kernel/cap_last_cap", O_RDONLY); fd >= 0 ) {
#ifdef HAVE_LINUX_MAGIC_H
			struct statfs st;
...
#endif
			char buf[8];
			if ( num = read(fd, buf, sizeof(buf) - 1); num > 0 ) {

				buf[num] = 0;
				errno = 0;
				unsigned int val = strtoul(buf, nullptr, 10);
...
static void init(void) {
	...
	(void)capget(&m.hdr, nullptr); // Returns -EINVAL
	if ( m.hdr.version == _LINUX_CAPABILITY_VERSION_3 || m.hdr.version == _LINUX_CAPABILITY_VERSION_2 ) {
		m.cap_ver = 3;
	...
	std::memset(&m.data, 0, sizeof(cap_data_t));
#ifdef HAVE_SYSCALL_H
...
void capng_clear(int set) {
	...
	if ( HAVE_PR_CAPBSET_DROP ) {
		if ( set & CAPNG_SELECT_BOUNDS )
			std::memset(m.bounds, 0, sizeof(m.bounds));
	}
#endif
#ifdef PR_CAP_AMBIENT
	if ( HAVE_PR_CAP_AMBIENT ) {
		if ( set & CAPNG_SELECT_AMBIENT )
			std::memset(m.ambient, 0, sizeof(m.ambient));
	}
#endif
	m.state = CAPNG_INIT;
}
...
void capng_fill(int set) {
	...
	if ( HAVE_PR_CAPBSET_DROP ) {
		if ( set & CAPNG_SELECT_BOUNDS ) {
			for ( unsigned i = 0; i < sizeof(m.bounds) / sizeof(__u32); i++ )
	...
	if ( HAVE_PR_CAP_AMBIENT ) {
		if ( set & CAPNG_SELECT_AMBIENT ) {
			for ( unsigned i = 0; i < sizeof(m.ambient) / sizeof(__u32); i++ )
...
static int get_ambient_set(void) {
	...
	do {
		...
		if ( rc = prctl(PR_CAP_AMBIENT, PR_CAP_AMBIENT_IS_SET, i, 0, 0); rc < 0 )
			return -1;
...
int capng_get_caps_process(void) {
	...
	if ( rc = capget((cap_user_header_t)&m.hdr, (cap_user_data_t)&m.data); rc == 0 ) {

		m.state = CAPNG_INIT;
#ifdef PR_CAPBSET_DROP

		if ( HAVE_PR_CAPBSET_DROP ) {
			if ( rc = get_bounding_set(); rc < 0 )
				m.state = CAPNG_ERROR;
		}
#endif
#ifdef PR_CAP_AMBIENT
		if ( HAVE_PR_CAP_AMBIENT ) {
			if ( rc = get_ambient_set(); rc < 0 )
				m.state = CAPNG_ERROR;
		}
#endif
	}
...
#ifdef VFS_CAP_U32
#ifdef VFS_CAP_REVISION_3
static int load_data(const vfs_ns_cap_data *filedata, int size)
#else
static int load_data(const vfs_cap_data *filedata, int size)
#endif
{
	unsigned int magic;
...
#ifdef VFS_CAP_REVISION_3
	if ( size == XATTR_CAPS_SZ_3 ) {
		vfs_ns_cap_data* d = (vfs_ns_cap_data*)filedata;
...
int capng_get_caps_fd(int fd) {
	...
	int rc;
#ifdef VFS_CAP_REVISION_3
	vfs_ns_cap_data filedata;
#else
	vfs_cap_data filedata;
#endif
	if ( m.state == CAPNG_NEW )
		init();
	if ( m.state == CAPNG_ERROR )
	...
	if ( rc = fgetxattr(fd, XATTR_NAME_CAPS, &filedata, sizeof(filedata)); rc <= 0 )
		return -1;

	if ( rc = load_data(&filedata, rc); rc == 0 )
		m.state = CAPNG_INIT;
	else
...
static void update_ambient_set(capng_act_t action, unsigned int capability, unsigned int idx) {
...
int capng_updatev(capng_act_t action, int type, unsigned int capability, ...) {
	...
	if ( rc = capng_update(action, type, capability); rc )
		return rc;
	...
	while ( cap_valid(cap)) {
		...
		if ( rc = capng_update(action, type, cap); rc )
			break;
...
int capng_apply(int set) {
	...
	if ( set & CAPNG_SELECT_BOUNDS ) {
		...
		if (HAVE_PR_CAPBSET_DROP) {
			...
			cap_ng state;
			std::memcpy(&state, &m, sizeof(state)); /* save state */
			...
			if ( capng_have_capability(CAPNG_EFFECTIVE, CAP_SETPCAP)) {
				...
				std::memcpy(&m, &state, sizeof(m)); /* restore state */

				for ( unsigned int i = 0; i <= last_cap; i++ ) {

					if ( capng_have_capability(CAPNG_BOUNDING_SET, i) == 0 ) {
...
try_caps:
	...
	if ( set & CAPNG_SELECT_CAPS ) {
		...
		if ( capset((cap_user_header_t)&m.hdr, (cap_user_data_t)&m.data) == 0 )
			m.state = CAPNG_APPLIED;
		else
			rc = -5;
	}
...
#ifdef VFS_CAP_U32
#ifdef VFS_CAP_REVISION_3
static int save_data(vfs_ns_cap_data *filedata, int *size)
#else
static int save_data(vfs_cap_data *filedata, int *size)
#endif
{
	// Now stuff the data structures
	if ( m.vfs_cap_ver == 1 ) {
...
if (HAVE_PR_CAPBSET_DROP) {
	...
	if ( capng_have_capability(CAPNG_EFFECTIVE, CAP_SETPCAP) == 0 && flag & CAPNG_CLEAR_BOUNDING )
	...
	if ( rc = capng_apply(CAPNG_SELECT_CAPS); rc < 0 ) {
		ret = -3;
		goto err_out;
	}
	...
	if ( flag & CAPNG_CLEAR_BOUNDING ) {
		...
		if ( rc = capng_apply(CAPNG_SELECT_BOUNDS); rc ) {
			ret = -8;
			goto err_out;
		}
	}
	...
	if ( gid != -1 ) {
		...
		if ( rc = setresgid(gid, gid, gid); rc ) {
			ret = -4;
			goto err_out;
		}
	}
	...
	if ( uid != -1 ) {
		...
		if ( rc = setresuid(uid, uid, uid); rc ) {
	...
	if ( rc = prctl(PR_SET_KEEPCAPS, 0, 0, 0, 0); rc )
		return -7;
	...
	if ( rc = capng_apply(CAPNG_SELECT_CAPS | CAPNG_SELECT_AMBIENT); rc < 0 )
		return -9;
	...
	if ( set & CAPNG_SELECT_CAPS ) {
		...
		if ( m.cap_ver == 1 ) {
			...
			if (( m.data.v3[1].effective & UPPER_MASK ) == 0 && !full )
				empty = 1;
			else if (( m.data.v3[1].effective & UPPER_MASK ) == UPPER_MASK && !empty )
				full = 1;
			else return CAPNG_PARTIAL;
		}
	}
